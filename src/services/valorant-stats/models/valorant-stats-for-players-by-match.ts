 ...
 */
export const valorantStatsForPlayersByMatch = z.object({
  match: match,
 ...
 */
export const valorantStatsForPlayersByMatchResponse = z
  .object({
    match: matchResponse,
 ...
 * Is equal to application shape if all property names match the api schema
 */
export const valorantStatsForPlayersByMatchRequest = z
  .object({
    match: matchRequest.nullish(),
    stats: z.array(valorantPlayerMatchStatsRequest).nullish(),
  })
  .transform((data) => ({
    match: data['match'],
    stats: data['stats'],
